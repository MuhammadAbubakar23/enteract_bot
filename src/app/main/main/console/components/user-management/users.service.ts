...
export class UsersService {
  ...
  GetUsers(formData:any) {
    return this.http.get(
      `${this.baseUrl}${"Users/GetAll"}?pageIndex=${formData.pageNumber}&pageSize=${formData.pageSize}&searchText=${formData.searchText}`
    );
  }

  GetUsersById(id:any)
  {
    return this.http.get(`${this.baseUrl}${"Users/GetAll"}?id=${id}}`);
  }

  DeleteUser(deleteId: string): Observable<any> {
    const url = `${this.baseUrl}${"User/Delete"}?Id=${deleteId}`;
    return this.http.get(url);
  }
  Save(form: any) {
    return this.http.post(this.baseUrl + "User/Save", form);
  }
